...
if (!$rerun):
	...
	while ($row = $res->fetch_assoc()) {
		$vetNames[] = $row['nome_vet'];
	}

	$vetByTime = [
		'10:00:00' => $vetNames[0] ?? 'Indisponível',
		'15:00:00' => $vetNames[1] ?? 'Indisponível',
		'18:00:00' => $vetNames[2] ?? 'Indisponível'
	];
?>
	...
	<body>
						...
						<div class="row column-gap-3 row-gap-3 justify-content-center">
								...
								<div class="rounded-bottom-4 p-4">
									<div class="row">
										...
										<div class="col-6">
											...
											<div>
												<input type="radio" name="horary" value="10:00:00" id="time_10" class="btn-check" onchange="updateVet()">
												<label for="time_10" class="time-label">10:00</label>
											</div>

											<div>
												<input type="radio" name="horary" value="15:00:00" id="time_15" class="btn-check" onchange="updateVet()">
												<label for="time_15" class="time-label">15:00</label>
											</div>

											<div>
												<input type="radio" name="horary" value="18:00:00" id="time_18" class="btn-check" onchange="updateVet()">
							...
							<div class="col-12 col-md-auto p-3">
								...
								<div class="rounded-bottom-4 d-flex flex-column p-4">
									...
									<p>Veterinário: <span id="vet-name"></span></p>
		...
		<script>
			const vetByTime = <?= json_encode($vetByTime); ?>;

			function updateVet() {
				const selectedTime = document.querySelector('input[name="horary"]:checked');
				const vetSpan = document.getElementById('vet-name');

				if (selectedTime) {
					vetSpan.textContent = vetByTime[selectedTime.value] || 'Indisponível';
				} else {
					vetSpan.textContent = '';
				}
			}

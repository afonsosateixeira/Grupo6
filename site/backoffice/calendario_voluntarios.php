...
?>
    <section class="ms-2">
        ...
        <!-- Grid de 7 colunas usando o Bootstrap -->
        <div class="row row-cols-7 g-3 m-0">
            <?php foreach ($scheduleByDay as $weekday => $turnos): ?>
                
                <div class="col d-flex flex-column gap-3">
                    
                    <!-- Cabeçalho do Dia (classe em PT) -->
                    <div class="cabecalho-dia p-2 text-center fw-bold border rounded-3">
                        <?= htmlspecialchars($displayDay[$weekday] ?? $weekday) ?>
                    </div>
                    
                    <?php if (empty($turnos)): ?>
                        <!-- Dia Vazio (classe em PT) -->
                        <div class="dia-vazio border border-dashed rounded-3 d-flex align-items-center justify-content-center bg-light text-secondary">
                            Sem turnos
                        </div>
                    <?php else: ?>
                        <!-- Lista de Turnos (classe em PT) -->
